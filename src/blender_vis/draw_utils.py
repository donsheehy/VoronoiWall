...
def draw_points(gp_frame, points, cyclic=True):
    gp_stroke = gp_frame.strokes.new()
    gp_stroke.display_mode = '3DSPACE'  # allows for editing
    ...
    for i in range(len(points)):
        gp_stroke.points[i].co = (points[i][0], points[i][1], points[i][2])

def test():
    #Basic code for getting grease pencil and layer instance
    ...
    draw_points(gp_frame, points)

    draw_circle(gp_frame, (-6, -1, 0), 2.0, 2000)
